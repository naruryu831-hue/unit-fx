...
2011年設立、ベリーズ拠点。MT4・MT5に加えて[cTrader](/articles/kaigai-fx-ctrader-taiou)を提供しており、cTraderは短期売買を行うトレーダーに選ばれることの多いプラットフォームです。約定方式にはNDD方式を採用していると案内されています。取引環境という観点では短期売買向けの選択肢が揃っている部類ですが、それはスキャルピングが規約上認められていることを意味しません。可否は必ず開設予定の口座タイプの規約で確認してください。

■ TitanFX（タイタンFX）｜ブレード口座という低スプレッド型の口座タイプがある

2014年設立。Zeroスタンダードとブレードの口座タイプがあり、ブレード口座はスプレッドを抑えて取引手数料を取る型の口座です。取引回数が増えるほどこのコスト構造の違いが効いてくるため、どちらの口座を選ぶかが実質的な検討ポイントになります。[レバレッジ](/articles/kaigai-fx-high-leverage-hikaku)はZeroマイクロ口座が最大2,000倍、Zeroスタンダード/ブレード口座は最大1,000倍。ボーナスは提供せず紹介プログラムで還元する方針のため、ボーナス絡みの取引制限を気にせず条件を比較できる点は確認が楽になります。

■ FXGT（FXGT.com）｜MT5対応、24時間の日本語サポート

2019年設立。MT5に対応し、FXに加えて暗号資産CFDも扱います。24時間対応の日本語サポートを謳っているため、規約の解釈を確認したいときに時間帯を選ばず問い合わせられるのは実務上の利点です。レバレッジは通常1:1000（Optimus口座は条件付で最大1:5000）、[最低入金額](/articles/kaigai-fx-saitei-nyukin-hikaku)は5〜10ドル程度。

■ Exness｜日本語ライブチャットで取引スタイルを直接確認しやすい

2008年設立。[日本語のライブチャット](/articles/kaigai-fx-nihongo-support)に対応しているため、自分の取引スタイルを伝えて規約上の扱いを尋ねるハードルが低い業者です。最大レバレッジは無制限（Unlimited）とされますが条件により変動するため、適用条件は公式サイトで確認してください。チャットで得た回答は口頭のやり取りと同じで後から追いにくいので、内容はスクリーンショットで残しておくことをおすすめします。
...
2013年設立、セントビンセント・グレナディーン諸島拠点。最大レバレッジ2222倍と高い水準を掲げています。少ない証拠金で回転させたい人の候補になりますが、レバレッジが高いほど短時間の値動きに対する損益の振れ幅も大きくなる点は前提として押さえておく必要があります。[口座タイプ](/articles/kaigai-fx-kouza-type-hikaku)ごとの取引条件、最低入金額、スキャルピングに関する規約上の扱いはいずれも公式サイトでの確認が必要です。
...
・口座タイプ比較ページで、[低スプレッド](/articles/kaigai-fx-spread-hikaku)＋手数料型（ECN/STP型）と手数料なし・スプレッド広め型（スタンダード型）のどちらが自分の取引回数でコストが低くなるかを試算する
