...
export const kaigaiFxKouzaBonusMatome: Article = {
  ...
  body: `[海外FX業者](/kaigai)の「口座開設ボーナス・入金ボーナス」について、公式情報から実施が確認できた業者を先に、確認できなかった業者を後に並べたまとめです。金額を推測で補うことはせず、確認できていないものは「要確認」と明示しています。
...
公式サイトで[口座開設ボーナスと入金ボーナスの両方](/articles/xm-bonus)が案内されている、数少ない業者のひとつです。
...
[「BigBoss Points」](/articles/bigboss-review)というポイントプログラムと、独自コインBigBoss Coin（BBC）建てで入金した際の10%相当のクレジットボーナスなど、複数のプロモーションを並行して実施しています。
...
ボーナスより[レバレッジ条件](/articles/kaigai-fx-high-leverage-hikaku)を優先したい人が、ボーナスの有無を公式サイトで確認したうえで候補に入れる、という順番になる業者です。

■ 2-2. HFM（HotForex）｜ボーナス：公式サイト参照

2010年設立。複数の金融ライセンスのもとでFX・貴金属・株価指数などのオンライン取引サービスを提供しています。日本語のライブチャット・メールサポートあり。

FX以外の銘柄も含めて一つの口座で扱いたい人向け。[ボーナスの実施有無](/articles/hfm-bonus)は公式サイトのプロモーションページで確認してください。

■ 2-3. FXGT（FXGT.com）｜ボーナス：公式サイト参照

2019年サービス開始の比較的新しい業者（運営会社GT Global Ltd、セーシェル金融庁ライセンス）。FXに加え暗号資産CFDを扱い、[最低入金額](/articles/kaigai-fx-saitei-nyukin-hikaku)は口座タイプにより5〜10ドル程度、24時間対応の日本語サポートを提供しています。
