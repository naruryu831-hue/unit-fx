...
export const metadata: Metadata = {
  title: `運営者情報・編集方針 | ${SITE_NAME}`,
  description: `${SITE_NAME}の運営者情報、編集方針、記事の作成・更新ルール、広告掲載に関する方針、免責事項をまとめたページです。`,
  alternates: { canonical: '/about' },
}

function buildSections(): { title: string; body: string[] }[] {
  const aboutBody =
    SITE_MARKET === 'overseas'
      ? `${SITE_NAME}は、海外FX業者の取引条件を「公式サイトで確認できる事実」だけで整理する比較サイトです。取引単位、スプレッドの表示方式、レバレッジ、口座開設の手順、税金の扱いといった、口座を開く前に知っておくべき情報を業者ごとに揃えて掲載しています。`
      : `${SITE_NAME}は、国内FX業者の取引条件を「公式サイトで確認できる事実」だけで整理する比較サイトです。取引単位、スプレッドの表示方式、レバレッジ、口座開設の手順、税金の扱いといった、口座を開く前に知っておくべき情報を業者ごとに揃えて掲載しています。`

  const registrationBody =
    SITE_MARKET === 'overseas'
      ? '海外FX業者については、日本の金融庁に登録されていない業者であることを毎回明示します。'
      : '国内FX業者については、金融商品取引業の登録番号を掲載します。'

  const leverageDisclaimer =
    SITE_MARKET === 'overseas'
